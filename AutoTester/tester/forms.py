...
class TesterForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.id:
            ...
            self.fields['maxPlungerDepthNoAgitator'].label="Max Plunger Drop in mm"
            self.fields['maxPlungerDepthNoAgitator'].widget.attrs['title'] = "Max plunge depth in mm.  To bottom out the syringe, but not overpush"
            self.fields['maxPlungerDepthAgitator'].label="Max Plunger Drop with Agitator Magnet"
            self.fields['maxPlungerDepthAgitator'].widget.attrs['title'] = "Max plunge depth in mm when syringe has an agitator.  To bottom out the syringe, but not overpush"
